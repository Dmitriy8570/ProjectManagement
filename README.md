...
презентациями (Vue 3 SPA + Web API и Razor MVC), EF Core Code-First с
миграциями и поддержкой SQLite/MSSQL.
...
фичу: `Projects/`, `Tasks/`, `Employees/`, `Documents/`).
...
┌─────────────────────────────────────────────────────────────┐
│  DataAccess                                                 │
│  AppDbContext (DbContext), EF Core Fluent configurations,   │
│  Repositories, Migrations, LocalFileStorage.                │
└─────────────────────────────────────────────────────────────┘
```

**Frontend (Vue SPA):** Vue 3 + `<script setup lang="ts">`, Pinia,
Vue Router, Axios; Vite-прокси на `/api` ходит на ASP.NET.
...
│   ├── DataAccess/              # AppDbContext, EF-конфигурации, миграции, репозитории, LocalFileStorage
│   ├── ProjectManagement.Api/   # Web API — серверный бэк для Vue SPA
│   └── ProjectManagement.Web/   # Razor MVC — самостоятельный фронт
...
│   ├── Tests.Presentation/      # E2E API через WebApplicationFactory
│   └── Tests.Presentation.Web/  # E2E Razor через WebApplicationFactory
...
|---|---|
| `Tests.BusinessLogic` | Доменные сущности (`Project`, `Employee`, `ProjectTask`, `ProjectDocument`), все Command/Query-хендлеры через NSubstitute |
| `Tests.DataAccess` | Репозитории на in-memory SQLite (фильтры, сортировка, пагинация, удаление с каскадом) |
| `Tests.Presentation` | E2E для Web API через `WebApplicationFactory<Program>` |
| `Tests.Presentation.Web` | E2E для Razor MVC через `WebApplicationFactory<Program>` |
