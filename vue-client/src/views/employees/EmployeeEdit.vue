...
async function save() {
  ...
  try {
    await employeesApi.update(id, {
      ...
      patronymic: patronymic.value.trim() || undefined, email: email.value.trim(),
    })
    notif.show('Employee updated.')
...
<template>
  <nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><RouterLink :to="`/employees/${id}`">{{ fullName }}</RouterLink></li>
  ...
  <div class="row justify-content-center">
    <div class="col-lg-6">
      <div class="card">
        <div class="card-header"><i class="bi bi-pencil-square me-2"></i>Edit Employee</div>
        <div class="card-body">
          ...
          <div class="d-flex gap-2 mt-4">
            ...
            <RouterLink :to="`/employees/${id}`" class="btn btn-outline-secondary">Cancel</RouterLink>
          </div>
        </div>
      </div>
    </div>
